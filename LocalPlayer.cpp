...
private:
    ...
    float m_lastCrosshairTime = 0; // Добавлено объявление переменной m_lastCrosshairTime
...
public:
    ...
    {
        long basePointer = getBasePointer();
        long ptrLong = basePointer + offsets::lastCrosshairTargetTime; //offsets::OFFSET_CROSSHAIR_LAST
        float result = mem::ReadFloat(ptrLong);
        return result;
    }
    bool isCrosshair()
